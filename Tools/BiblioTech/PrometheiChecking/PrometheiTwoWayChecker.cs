...
namespace BiblioTech.PrometheiChecking
{
    public interface ICheckResponseHandler
    {
        ...
        Task PrometheiUnavailable();

        Task ToAdminChannel(string msg);
    }

    public class PrometheiTwoWayChecker
    {
        ...
        public PrometheiTwoWayChecker(ILog log, Configuration config, CheckRepo repo, PrometheiWrapper prometheiWrapper)
        {
            this.log = log;
            this.config = config;
            this.repo = repo;
            this.prometheiWrapper = prometheiWrapper;
        }
        ...
        public async Task StartDownloadCheck(ICheckResponseHandler handler, ulong userId)
        {
            ...
            if (cid == null)
            {
                await handler.PrometheiUnavailable();
                return;
            }
        ...
        private string? UploadData(string uniqueData)
        {
            ...
            try
            {
                ...
                return prometheiWrapper.OnPromethei(node =>
                {
        ...
        private bool? IsContentCorrect(ICheckResponseHandler handler, TransferCheck check, string receivedCid)
        {
            try
            {
                var content = prometheiWrapper.OnPromethei(node =>
                {
