...
{
    public interface IPrometheiNodeHooks
    {
        void OnNodeStarting(DateTime startUtc, string image, EthAccount? ethAccount);
        void OnNodeStarted(IPrometheiNode node, string peerId, string nodeId);
        void OnNodeStopping();
    ...
    public class MuxingPrometheiNodeHooks : IPrometheiNodeHooks
    {
        private readonly IPrometheiNodeHooks[] backingHooks;

        public MuxingPrometheiNodeHooks(IPrometheiNodeHooks[] backingHooks)
        {
