...
namespace PrometheiContractsPlugin
{
    public class ContractsContainerInfoExtractor
    {
        ...
        public ContractAddress ExtractMarketplaceAddress()
        {
            ...
            if (string.IsNullOrEmpty(marketplaceAddress)) throw new InvalidOperationException("Unable to fetch marketplace account from promethei-contracts node. Test infra failure.");
        ...
        public (string, string) ExtractMarketplaceAbiAndByteCode()
        {
            ...
            if (string.IsNullOrEmpty(abi)) throw new InvalidOperationException("Unable to fetch marketplace artifacts from promethei-contracts node. Test infra failure.");
        ...
        {
            var json = workflow.ExecuteCommand(container, "cat", PrometheiContractsContainerRecipe.DeployedAddressesFilename);
            json = json.Replace("#", "_");
        ...
        {
            var json = workflow.ExecuteCommand(container, "cat", PrometheiContractsContainerRecipe.MarketplaceArtifactFilename);
