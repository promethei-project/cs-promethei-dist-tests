...
namespace PrometheiContractsPlugin
{
    public class SelfUpdater
    {
        public void Update(string abi, string bytecode)
        {
            ...
            throw new Exception("Oh no! PrometheiContracts were updated. Current build of PrometheiContractsPlugin is incompatible. " +
                "But fear not! SelfUpdater.cs has automatically updated the plugin. Just rebuild and rerun and it should work. " +
                "Just in case, manual update instructions are found here: 'PrometheiContractsPlugin/Marketplace/README.md'.");
        }

        private string GetMarketplaceFilePath()
        {
            var projectPluginDir = PluginPathUtils.ProjectPluginsDir;
            var path = Path.Combine(projectPluginDir, "PrometheiContractsPlugin", "Marketplace", "Marketplace.cs");
