...
{
    public class PrometheiDiscordBotPlugin : IProjectPlugin, IHasLogPrefix, IHasMetadata
    {
        private const string ExpectedStartupMessage = "Debug option is set. Discord connection disabled!";
        private readonly IPluginTools tools;

        public PrometheiDiscordBotPlugin(IPluginTools tools)
        {
            this.tools = tools;
        }
        ...
        {
            tools.GetLog().Log($"Promethei DiscordBot (BiblioTech) loaded.");
        }

        public void AddMetadata(IAddMetadata metadata)
        {
            metadata.Add("prometheidiscordbotid", new DiscordBotContainerRecipe().Image);
        }
