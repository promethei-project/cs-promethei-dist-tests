...
using Logging;
using Utils;
using static PrometheiTests.Helpers.FullConnectivityHelper;

namespace PrometheiTests.Helpers
{
    public class PeerDownloadTestHelpers : IFullConnectivityImplementation
    {
        ...
        public void AssertFullDownloadInterconnectivity(IEnumerable<IPrometheiNode> nodes, ByteSize testFileSize)
        {
        ...
        private TrackedFile? DownloadFile(IPrometheiNode node, ContentId contentId, string label)
        {
            return node.DownloadContent(contentId, downloadTimeout, label);
        }

        private TrackedFile GenerateTestFile(IPrometheiNode uploader, IPrometheiNode downloader)
        {
